...
export default function DashboardPage() {
  ...
  if (showNickname) return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <form onSubmit={handleSetNickname} className="bg-white dark:bg-zinc-800 rounded-xl p-6 w-full max-w-sm flex flex-col gap-4 shadow-sm">
        <h2 className="text-lg font-semibold">닉네임 설정</h2>
        <p className="text-sm text-gray-500 dark:text-zinc-400">그룹에서 표시될 이름을 정해주세요.</p>
        <input
          value={nickname}
          ...
          required
          maxLength={20}
          className="bg-gray-100 dark:bg-zinc-700 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button
          type="submit"
          disabled={nicknameSaving}
          className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-lg py-2 text-sm font-semibold transition-colors text-white"
        >
  ...
  return (
    <div className="min-h-screen relative px-4 py-6">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="text-sm bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-lg px-3 py-1.5 transition-colors"
              title="테마 변경"
            >
              {theme === 'dark' ? '☀️' : '🌙'}
            </button>
            <Link href="/groups" className="text-sm bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-lg px-3 py-1.5 transition-colors">
              👥 그룹
            </Link>
            {isAdmin && (
              <Link href="/admin" className="text-sm bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-lg px-3 py-1.5 transition-colors">
                🛠️
              </Link>
            )}
            <button onClick={() => { signOut(); router.replace('/login') }} className="text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-800 dark:hover:text-zinc-200">
              로그아웃
            </button>
          </div>
        </div>
        ...
        {/* 총계 카드 */}
        <div className="bg-white dark:bg-zinc-800 rounded-xl p-5 mb-4 shadow-sm dark:shadow-none">
          <div className="flex items-center justify-between mb-1">
            <p className="text-sm text-gray-500 dark:text-zinc-400">총 평가금액</p>
            <div className="flex gap-1.5">
              <button
                ...
                className="text-xs bg-gray-100 dark:bg-zinc-700 hover:bg-gray-200 dark:hover:bg-zinc-600 rounded-full px-2 py-1 transition-colors"
                title="금액 숨기기"
              >
                {hideAmounts ? '👁' : '🙈'}
              </button>
              <button
                ...
                className="text-xs bg-gray-100 dark:bg-zinc-700 hover:bg-gray-200 dark:hover:bg-zinc-600 rounded-full px-2 py-1 transition-colors font-semibold"
              >
                {currency === 'USD' ? '$ → ₩' : '₩ → $'}
              </button>
            </div>
          ...
          <p className={`mt-1 text-sm font-semibold ${totalPnl >= 0 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
            {totalPnl >= 0 ? '+' : ''}{fmt(totalPnl)}
            {' '}({totalPnlPct >= 0 ? '+' : ''}{totalPnlPct.toFixed(2)}%)
          </p>
          {currency === 'KRW' && (
            <p className="mt-1 text-xs text-gray-400 dark:text-zinc-500">환율 ₩{Math.round(exchangeRate).toLocaleString()}/$ 기준</p>
          )}
        </div>
        ...
        <div className="xl:hidden mb-4 -mx-4 px-4">
          <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
            ...
            {/* 주요 지수 */}
            {indices.map(idx => (
              <div key={idx.symbol} className="flex-shrink-0 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl px-3 py-2.5">
                <p className="text-xs text-gray-400 dark:text-zinc-500">{INDEX_LABELS[idx.symbol] ?? idx.symbol}</p>
                <p className="text-sm font-bold font-mono">
                  {idx.price.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                </p>
                <p className={`text-xs font-semibold ${idx.changePercent >= 0 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
        ...
        {holdings.length === 0 ? (
          <div className="bg-white dark:bg-zinc-800 rounded-xl p-8 text-center shadow-sm dark:shadow-none">
            <div className="text-5xl mb-4">📊</div>
            <h3 className="font-semibold text-base mb-1">아직 보유 종목이 없어요</h3>
            <p className="text-sm text-gray-500 dark:text-zinc-400 mb-5">첫 종목을 추가해서 포트폴리오를 시작해보세요.</p>
            <button
              onClick={() => { setShowAdd(true); setSearchQuery(''); setSearchResults([]); setForm({ symbol: '', quantity: '', avg_price: '', total_invested: '' }); setSelectedPrice(null); setPriceMode('avg') }}
              className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-5 py-2 text-sm font-semibold transition-colors mb-8"
            >
              + 첫 종목 추가하기
            </button>
            <div className="grid grid-cols-3 gap-3 text-xs text-gray-400 dark:text-zinc-500 border-t border-gray-200 dark:border-zinc-700 pt-6">
              <div className="flex flex-col items-center gap-2">
                <span className="text-2xl">🔍</span>
                <p className="font-semibold text-gray-700 dark:text-zinc-300">티커·회사명 검색</p>
                <p>AAPL, 애플, NVDA 등<br />뭐든 검색 가능해요</p>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="text-2xl">👥</span>
                <p className="font-semibold text-gray-700 dark:text-zinc-300">그룹 수익률 비교</p>
                <p>친구들과 그룹을 만들어<br />수익률을 겨뤄보세요</p>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="text-2xl">📅</span>
                <p className="font-semibold text-gray-700 dark:text-zinc-300">히스토리 추적</p>
          ...
          <div className="flex flex-col gap-3">
            {holdings.map(h => {
              ...
              return (
                <div
                  key={h.id}
                  className="bg-white dark:bg-zinc-800 rounded-xl p-5 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-700/50 hover:ring-1 ring-gray-200 dark:ring-zinc-600 transition-all shadow-sm dark:shadow-none"
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {h.quote?.name && <p className="text-xs text-gray-400 dark:text-zinc-500 mt-0.5">{h.quote.name}</p>}
                    </div>
                    <div className="text-right">
                      <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${(h.quote?.changePercent ?? 0) >= 0 ? 'bg-green-400/10 text-green-600 dark:text-green-400' : 'bg-red-400/10 text-red-600 dark:text-red-400'}`}>
                        {(h.quote?.changePercent ?? 0) >= 0 ? '+' : ''}{h.quote?.changePercent?.toFixed(2) ?? '0'}%
                      </span>
                      <p className="text-xs text-gray-300 dark:text-zinc-600 mt-1">오늘</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    <div className="text-right">
                      <p className={`text-xl font-bold tabular-nums ${pnl >= 0 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
                        {pnl >= 0 ? '+' : ''}{fmt(pnl)}
                      </p>
      ...
      {editingHolding && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center px-4 z-50" onClick={() => setEditingHolding(null)}>
          <form onSubmit={handleEditSave} className="bg-white dark:bg-zinc-800 rounded-xl p-6 w-full max-w-sm flex flex-col gap-4 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-semibold">{editingHolding.symbol} 수정</h3>
              {editingHolding.quote?.price && (
                <span className="text-xs text-gray-500 dark:text-zinc-400">현재가 {fmt(editingHolding.quote.price)}</span>
              )}
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-sm text-gray-500 dark:text-zinc-400">수량 (주)</label>
              <input
                type="number"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-sm text-gray-500 dark:text-zinc-400">평단가 ($)</label>
              <input
                type="number"
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => handleDelete(editingHolding.id)} className="bg-red-600/20 hover:bg-red-600/40 text-red-500 dark:text-red-400 rounded-lg py-2 px-3 text-sm transition-colors">삭제</button>
              <button type="button" onClick={() => setEditingHolding(null)} className="flex-1 bg-gray-100 dark:bg-zinc-700 hover:bg-gray-200 dark:hover:bg-zinc-600 rounded-lg py-2 text-sm transition-colors">취소</button>
              <button type="submit" disabled={editSaving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-lg py-2 text-sm font-semibold transition-colors text-white">
                {editSaving ? '저장 중...' : '저장'}
              </button>
            </div>
          </form>
      ...
      {showOnboarding && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
            <div className="bg-white dark:bg-zinc-800 rounded-2xl p-7 w-full max-w-sm flex flex-col gap-5 shadow-2xl">
              {/* 진행 점 */}
              <div className="flex justify-center gap-1.5">
                {steps.map((_, i) => (
                  <div key={i} className={`h-1.5 rounded-full transition-all ${i === onboardingStep ? 'w-6 bg-blue-500' : 'w-1.5 bg-gray-300 dark:bg-zinc-600'}`} />
                ))}
              </div>
              {/* 내용 */}
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 dark:text-zinc-300 leading-relaxed mb-2">{step.desc}</p>
                {step.sub && <p className="text-xs text-gray-400 dark:text-zinc-500">{step.sub}</p>}
              </div>
              {/* 버튼 */}
              <div className="flex gap-2">
                <button onClick={close} className="text-xs text-gray-400 dark:text-zinc-500 hover:text-gray-700 dark:hover:text-zinc-300 px-3 py-2 transition-colors">
                  건너뛰기
                </button>
                <button
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center px-4 z-50">
          <form onSubmit={handleAddHolding} className="bg-white dark:bg-zinc-800 rounded-xl p-6 w-full max-w-sm flex flex-col gap-4 shadow-xl">
            <h3 className="font-semibold">종목 추가</h3>
            <div className="flex flex-col gap-1">
              <label className="text-sm text-gray-500 dark:text-zinc-400">종목 검색</label>
              <input
                value={searchQuery}
              ...
              {(searchResults.length > 0 || searching) && (
                <div className="bg-gray-100 dark:bg-zinc-700 rounded-lg overflow-hidden">
                  {searching && <p className="px-3 py-2 text-xs text-gray-400 dark:text-zinc-400">검색 중...</p>}
                  {searchResults.map(r => (
                    <button
                      key={r.symbol}
                      type="button"
                      ...
                      className="w-full text-left px-3 py-2 text-sm hover:bg-gray-200 dark:hover:bg-zinc-600 flex justify-between"
                    >
                      <span className="font-semibold">{r.symbol}</span>
                      <span className="text-gray-400 dark:text-zinc-400 text-xs truncate ml-2">{r.name}</span>
                    </button>
                  ))}
                </div>
              )}
              {form.symbol && (
                <div className="flex items-center justify-between">
                  <p className="text-xs text-green-600 dark:text-green-400">선택됨: {form.symbol}</p>
                  {selectedPrice !== null && (
                    <p className="text-xs text-gray-400 dark:text-zinc-400">현재가: <span className="font-semibold">{fmt(selectedPrice)}</span></p>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-zinc-700 rounded-lg p-1">
              <button
                ...
                className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${priceMode === 'avg' ? 'bg-white dark:bg-zinc-500 font-semibold shadow-sm' : 'text-gray-400 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-200'}`}
              >
                평단가 입력
              </button>
              <button
                ...
                className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${priceMode === 'total' ? 'bg-white dark:bg-zinc-500 font-semibold shadow-sm' : 'text-gray-400 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-200'}`}
              >
                투자 원금 입력
              </button>
            </div>
            ...
            {priceMode === 'avg' ? (
              <div className="flex flex-col gap-1">
                <label className="text-sm text-gray-500 dark:text-zinc-400">매수 평단가 ($)</label>
                <input
                  type="number"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-sm text-gray-500 dark:text-zinc-400">총 투자 원금 ($)</label>
                <input
                  type="number"
                ...
                {form.total_invested && form.quantity && (
                  <p className="text-xs text-gray-400 dark:text-zinc-400 mt-0.5">
                    → 평단가: <span className="font-semibold">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setShowAdd(false); setSearchQuery(''); setSearchResults([]); setSelectedPrice(null) }} className="flex-1 bg-gray-100 dark:bg-zinc-700 hover:bg-gray-200 dark:hover:bg-zinc-600 rounded-lg py-2 text-sm transition-colors">취소</button>
              <button type="submit" disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-lg py-2 text-sm font-semibold transition-colors">
                {saving ? '저장 중...' : '추가'}
              </button>
            </div>
          </form>
